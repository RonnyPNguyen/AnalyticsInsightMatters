...
def lambda_handler(event, context):
    ...
    industryKeys = {
        ...
        "Education and training (private)": "P",
        "Health care and social assistance (private)": "Q",
    ...
    with open("./ownerSalary.json", "r", encoding="utf-8") as f:
        ownerSalary = json.load(f)
    ...
    marketData = {
        ...
        "rentalYieldByCapital": {c: rentalYields[c] for c in rentalYields},
        "ownerSalary": ownerSalary,
    }

    s3 = boto3.client('s3')
    s3.put_object(
        Bucket='aim-project-server',  # replace with your bucket name
        Key='data/marketData.json',  # or any path inside the bucket
        Body=json.dumps(marketData),
...
# zip -r lambda-getMarketData . && aws s3 cp lambda-getMarketData.zip s3://aim-project-server/scripts/lambda-getMarketData